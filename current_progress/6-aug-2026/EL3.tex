...
Berdasrkan ISO 29148-2018\cite{3.1.1}, spesifikasi sistem ditetapkan sebagai acuan 
...
yang mampu dilaksanakan oleh sistem\cite{3.1.1}. Tabel \ref{tab:fungsional} menunjukkan
...
Pola getaran mampu mengindikasikan adanya pemerosotan kondisi mekanis pada motor \cite{1.1.1},
efektivitas parameter getaran dalam memprediksi adanya gangguan mekanis terbukti
lebih baik dibandingkan dengan parameter arus\cite{1.1.2}. Maka dari itu, fungsi utama
...
parameter suhu biasanya adalah sistem proteksi\cite{3.1.2}. Peningkatan
suhu pada mesin listrik disebabkan oleh kegagalan sistem pendingin
dan arus berlebih pada winding rotor dan/atau stator\cite{3.1.3}. 
...
rating daya di bawah 15kW\cite{IEC60034} (poin 2). 
...
\textit{overcurrent} pada pada motor\cite{3.1.4}. Selain sebagai komponen
sistem proteksi, parameter arus motor juga dapat digunakan sebagai pendukung diagnosa
kerusakan mekanis pada motor\cite{1.1.2}. 
...
parameter \textit{Motor Health Index} (MHI) (poin 12),
parameter MHI berguna untuk indikasi tingkat kesehatan motor
secara keseluruhan dan perhitungan sisa umur motor
...
untuk merekam data pada sistem\cite{3.1.6}. Sebelum sinyal transduser
diproses lebih lanjut pada mikrokontroler,
transduser harus terlebih dahulu dikalibrasi\cite{3.1.6}.
Proses kalibrasi merupakan proses pembandingan kualitas
data transduser dengan alat ukur yang telah ditetapkan\cite{3.1.6}.
Hasil proses kalibrasi harus didokumentasikan untuk memastikan
kesesuaian, kekonsistenan, dan validitas dari hasil ukur transduser\cite{3.1.6}.

%Mengapa cloud computing?
Selanjutnya, sistem dirancang untuk mendukung arsitektur
\textit(cloud computing) dengan memanfaatkan Wi-Fi sebagai media
transmisi data menuju server (poin 5). Untuk meningkatkan keandalan,
data juga disimpan secara lokal menggunakan media penyimpanan eksternal
sebagai mekanisme backup storage apabila terjadi
gangguan komunikasi (poin 14). Data historis disimpan pada basis
data serverless sehingga riwayat operasi motor dapat diakses untuk
keperluan analisis jangka panjang dan keperluan evaluasi 
motor (poin 9).
...
spesifikasi fungsional\cite{3.1.1}. Tabel \ref{tab:nonfungsional} mewakili
...
\subsubsection{Perhitungan Analisis Domain Waktu}
    ...
    kecepatan seperti pada persamaan \ref{eq:velocity}\cite{2.1.3}.
    ...
    mengkarakterisasi respon getaran mesin \cite{2.1.3}.
...
\subsubsection{Perhitungan Analisis Domain Frekuensi}
    ...
    ISO 13373-1 lampiran C \cite{2.1.4}.
...
\subsubsection{Perhitungan Batas Suhu Housing Maksimum}
    ...
    \begin{table}[H]
    \centering
    \caption{Tabel kelas insulasi winding\cite{3.1.5}}
...
% \subsubsection{Pemrosesan Sinyal Sensor Accelerometer ADXL345}
% Kalman filter umum digunakan untuk analisis getaran.

% \subsubsection{Pemrosesan Sinyal Sensor CT}

% \subsubsection{Pemrosesan Sinyal Termokapel}

% \subsubsection{Pemrosesan Sinyal Sensor Efek-Hall}
...
PCB  pada gambar \ref{fig:layout-pcb-isps},
...
\begin{figure}[H]
    ...
    \caption{Layout PCB Subsistem Instalasi Sensor dan Pemrosesan Sinyal (ISPS)}
    \label{fig:layout-pcb-isps}
\end{figure}
...
proses transmisi. Flowchart \ref{fig:flowchart_kca} menunjukkan
...
\subsubsection{Desain Tampilan Antarmuka Sistem Pemantauan}
