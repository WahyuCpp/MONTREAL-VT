...
kondisi motor seperti yang tercantum pada tabel \ref{tab:fungsional}.
Spesifikasi ini mendefinisikan seluruh fungsi
yang harus dipenuhi sistem agar mampu melaksanakan proses
akuisisi data, pemrosesan sinyal, komunikasi data, 
analisis kondisi, visualisasi, serta mekanisme proteksi
secara terpadu. 
...
\begin{longtable}{|p{1.5cm}|p{7cm}|p{5cm}|}
    ...
    1 
    & Sistem harus mampu mengukur parameter 
    akselerasi getaran 
    & \textit{Vibration analysis}\\
    \hline

    2 
    & Sistem harus mampu mengukur suhu housing 
    motor
    ...
    3 
    & Sistem harus mampu mengukur arus motor 
    & Deteksi overcurrent
    ...
    4 
    & Sistem harus mampu mengukur kecepatan rotasi shaft 
    & Deteksi perubahan kecepatan dan slip pada
    shaft \\
    \hline

    5 
    & Sistem harus mampu mengkondisikan sinyal 
    sensor sebelum atau disaat dikirimkan ke
    mikrokontroler 
    ...
    time-stamp pada setiap parameter ukur 
    & Sinkronisasi parameter ukur \\
    \hline

    7 
    & Sistem harus mampu mentransmisikan data
     menuju cloud/server menggunakan Wi-Fi 
     & \textit{Cloud computing} \\
    \hline

    8 
    & Sistem harus dilengkapi dengan metode
    penyimpanan data eksternal (SD Card)
    & \textit{Backup storage}\\
    \hline
    
    9
    & Sistem harus mampu menyimpan data historis
    dalam basis data serverless
    & Analisis data historis \\
    \hline

    10
    & Sistem harus mampu memroses data getaran;
    ...
    11 
    & Sistem harus mampu membandingkan pola getaran
    pada analisis frekuensi dengan data referensi
    & \textit{Fault prediction} \\
    \hline

    12 
    & Sistem harus mampu menghitung parameter
    \textit{Motor Health Index} (MHI) 
    & \textit{Condition rating} \\
    \hline

    13
    & Sistem harus mampu menampilkan data akuisisi real-time pada 
    dashboard 
    & \textit{Remote condition monitoring} \\
    \hline

    14 
    & Sistem harus mampu menampilkan tren historis getaran, arus, 
    suhu, dan rotasi shaft 
    & \textit{Remote condition monitoring} \\
    \hline

    15
    & Sistem harus dilengkapi dengan mekanisme
    ...
    16 
    & Sistem harus dilengkapi akses histori
    alarm dan perawatan 
    & \textit{Maintenance logging} \\
    \hline
\end{longtable}

Fungsi utama sistem paling utama tercantum seperti pada poin 1-4
adalah kemampuan sistem melakukan pembacaan, pengolahan,
parameter operasional motor yang telah ditetapkan, yakni akselerasi getaran
suhu housing, arus motor, dan kecepatan rotasi shaft. Parameter
tersebut merepresentasikan indikator utama kesehatan motor.

Data getaran digunakan untuk mendeteksi kerusakan mekanis 
(contoh: unbalance, misalignment, bearing fault), sedangkan data
suhu housing berfungsi sebagai identfikasi peningkatan suhu
akibat kerusakan winding. Pengukuran arus dimanfaatkan untuk
memantau kondisi pembebanan dan deteksi overcurrent, sementara kecepatan rotasi
digunakan untuk mengidentifikasi adanya perubahan kecepatan
maupun slip yang mampu mengindikasikan gangguan mekanis atau
elektrik pada motor.

Sebelum data diproses lebih lanjut pad mikrokontroler,
sistem harus mampu melakukan pengkondisian sinyal
(\textit{signal conditioning}) (poin 5) sebelum dikirimkan atau 
pada mikrokontroler agar kualitas sinyal sesuai dengan
spesifikasi tegangan pin mikrokontroler.
Sistem juga harus mampu memberikan \textit{time-stamp}
pada setiap parameter ukur untuk menjamin
sinkronisasi data antarparameter sehingga data
dapat dianalisis secara akurat (poin 6).

Selanjutnya, sistem dirancang untuk mendukung arsitektur
cloud computing dengan memanfaatkan Wi-Fi sebagai media
transmisi data menuju server (poin 7). Untuk meningkatkan keandalan,
data juga disimpan secara lokal menggunakan media penyimpanan eksternal
sebagai mekanisme backup storage apabila terjadi
gangguan komunikasi (poin 8). Data historis disimpan pada basis
...
Pada tahap pemrosesan tingkat lanjut, sistem 
harus mampu mengolah data akselerasi getaran, hal ini meliputi
proses konversi data akselerasi menjadi kecepatan getaran dan
kecepatan rms, serta transformasi sinyal dari domain waktu
ke domain frekuensi menggunakan metode Fast Fourier Transform (FFT) (poin 10).
Hasil analisis domain frekuensi data getaran 
akan dibandingkan dengan data referensi untukm
endeteksi adanya pola getaran yang mengindikasikan
kerusakan mekanis pada motor (poin 11).
...
Sebagai komponen antarmuka pengguna, sistem harus mampu
menyajikan data hasil pengukuran secara real-time pada dashboard (poin 13),
serta menampilkan tren historis dari seluruh parameter yang dipantau (poin 14).
Penyajian data historis bertujuan agar pengguna dapat mengakses
data historis motor untuk keperluan analisis perkembangan kondisi motor,
sehingga proses pengambilan keputusan pemeliharaan 
dapat dilakukan dengan lebih objektif.

Terakhir, sistem harus dilengkapi dengan mekanisme 
alarm dan proteksi yang akan aktif apabila parameter operasi arus dan suhu
melampaui batas yang telah ditentukan (poin 14-15). 
Selain memberikan notifikasi kondisi motor abnormal, sistem
harus menyediakan fiture pencatatan histori alarm dan aktivitas perawatan
(\textit{maintenance logging}). Dokumentasi tersebut berfungsi
sebagai rekam jejak operasional motor yang dapat dimanfaatkan
untuk evaluasi efektivitas pemeliharaan, analisis akar gangguan, serta
perencanaan predictive maintenance di masa mendatang (poin 16).
