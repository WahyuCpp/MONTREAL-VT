...
Berdasrkan ISO 29148-2018\cite{ISO2018DesignReq}, spesifikasi sistem ditetapkan sebagai acuan 
...
yang mampu dilaksanakan oleh sistem\cite{ISO2018DesignReq}. Tabel \ref{tab:fungsional} menunjukkan
...
Pola getaran mampu mengindikasikan adanya pemerosotan kondisi mekanis pada motor \cite{Vishnu2015Vibration},
efektivitas parameter getaran dalam memprediksi adanya gangguan mekanis terbukti
lebih baik dibandingkan dengan parameter arus\cite{Ayankoso2024VibrationCurrent}. Maka dari itu, fungsi utama
...
seperti transduser efek hall\cite{A3144} dan transduser arus\cite{ACS712}, 
...
parameter suhu biasanya adalah sistem proteksi\cite{Maximiliano2010SensorlessIM}. Peningkatan
suhu pada mesin listrik disebabkan oleh kegagalan sistem pendingin
dan arus berlebih pada winding rotor dan/atau stator\cite{Bonnett1991Failures}. 
...
rating daya di bawah 15kW\cite{iec60034_2022} (poin 2). 
...
\textit{overcurrent} pada pada motor\cite{Kishibe2000Patent}. Selain sebagai komponen
sistem proteksi, parameter arus motor juga dapat digunakan sebagai pendukung diagnosa
kerusakan mekanis pada motor\cite{Ayankoso2024VibrationCurrent}. 
...
Life}) atau RUL \cite{Yang2016MHI}. 
...
untuk merekam data pada sistem\cite{ISO10012Measurement}. Sebelum sinyal transduser
diproses lebih lanjut pada mikrokontroler,
transduser harus terlebih dahulu dikalibrasi\cite{ISO10012Measurement}.
Proses kalibrasi merupakan proses pembandingan kualitas
data transduser dengan alat ukur yang telah ditetapkan\cite{ISO10012Measurement}.
Hasil proses kalibrasi harus didokumentasikan untuk memastikan
kesesuaian, kekonsistenan, dan validitas dari hasil ukur transduser\cite{ISO10012Measurement}.
...
spesifikasi fungsional\cite{ISO2018DesignReq}. Tabel \ref{tab:nonfungsional} mewakili
...
\subsubsection{Perhitungan Analisis Domain Waktu}
    ...
    kecepatan seperti pada persamaan \ref{eq:velocity}\cite{BS20816ISO}.
    ...
    mengkarakterisasi respon getaran mesin \cite{BS20816ISO}.
...
\subsubsection{Perhitungan Analisis Domain Frekuensi}
    ...
    ISO 13373-1 lampiran C \cite{ISO13373V1}.
...
\subsubsection{Perhitungan Batas Suhu Housing Maksimum}
    ...
    \ref{tab:insulation_class}\cite{iec60034_2022}.
    
    \begin{table}[H]
    \centering
    \caption{Tabel kelas insulasi winding\cite{Swardt2022Insulation}}
...
yang dikembangkan \cite{oppenheim2010,Filho2025Fault,mqtt311}.
...
\begin{sidewaysfigure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{image/doc/skematik-monitor-2.pdf} % Ganti dengan path diagram Anda
...
dilakukan dengan menggunakan aplikasi KiCAD.


% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{image/doc/pcb-layout.pdf} % Ganti dengan path diagram Anda
%     \caption{Layout PCB ISPS}
%     \label{fig:pcb-layer}
% \end{figure}
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.8\textwidth]{image/doc/device-motor.jpg} % Ganti dengan path diagram Anda
%     \caption{Lokasi Instalasi Device}
%     \label{fig:instalasi-device}
% \end{figure}
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
Algoritma FFT dan pemrosesan sinyal ditulis menggunakan bahasa 
...
motor near-real-time, memliki latensi end-to-end kurang dari 500ms
...
\begin{enumerate}
    ...
    melalui 
    \item \textbf{Alasan Penolakan:} Implementasi operasi cloud-computing ...
\end{enumerate} 
...
\textbf{Kegiatan:}
\begin{itemize}
    \item Studi mengenai motor induksi dan mekanisme kerusakannya \cite{Nandi2005Condition}.
    \item Studi karakteristik sensor ADXL345, DS18B20, ACS712, dan Hall Effect Sensor \cite{ADXL345,ACS712,A3144}.
    \item Studi metode pemrosesan sinyal seperti Filtering, RMS, dan FFT \cite{oppenheim2010,Filho2025Fault}.
    \item Studi sistem monitoring kondisi motor dan sistem proteksi \cite{Jardine2006Machinery}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Mendesain algoritma RMS dan FFT \cite{oppenheim2010,Filho2025Fault}.
...
\begin{itemize}
    ...
    Tahap pengujian dilakukan untuk memastikan seluruh fungsi perangkat keras maupun perangkat lunak berjalan sesuai spesifikasi yang telah dirancang. Pengujian meliputi validasi sensor, analisis sinyal, komunikasi data, hingga pengujian integrasi sistem secara menyeluruh sehingga sistem memenuhi kebutuhan monitoring kondisi motor induksi \cite{PMBOK,Jardine2006Machinery}.
\end{itemize}
...
Rencana jadwal implementasi penelitian ditunjukkan pada tabel \ref{fig:gantt}.
\begin{sidewaysfigure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{image/doc/gantt-2.pdf} % Ganti dengan path diagram Anda
...
Sistem yang dikembangkan mampu melakukan monitoring kondisi motor induksi secara \textit{real-time} melalui pengukuran parameter getaran, suhu, arus, dan kecepatan putar motor. Data hasil pengukuran diproses menggunakan metode \textit{Root Mean Square} (RMS) dan \textit{Fast Fourier Transform} (FFT) sehingga mampu memberikan informasi mengenai kondisi operasi motor secara lebih akurat \cite{Filho2025Fault,oppenheim2010}.

sistem ini juga dilengkapi dengan algoritma klasifikasi kondisi motor, perhitungan \textit{Motor Health Index}, alarm, dan sistem proteksi sehingga pengguna dapat memperoleh peringatan dini terhadap potensi kerusakan sebelum terjadi kegagalan operasi (\textit{breakdown}) \cite{Jardine2006Machinery}.
\subsubsection{Manfaat Ekonomi}

Implementasi sistem monitoring memberikan potensi penghematan biaya pemeliharaan melalui penerapan konsep \textit{predictive maintenance}. Dengan adanya informasi kondisi motor secara \textit{real-time}, tindakan perawatan dapat dilakukan sebelum terjadi kerusakan yang lebih serius sehingga biaya perbaikan maupun penggantian komponen dapat diminimalkan \cite{Jardine2006Machinery}.
...
Keberadaan sistem alarm dan proteksi meningkatkan keselamatan operasi dengan memberikan peringatan ketika parameter getaran, suhu, maupun arus melebihi nilai ambang (\textit{threshold}) yang telah ditentukan \cite{iec60034_2022,Jardine2006Machinery}.

\subsubsection{Manfaat Akademik}

Pengembangan sistem ini memberikan kontribusi terhadap penelitian mengenai \textit{condition monitoring} dan \textit{predictive maintenance} pada motor induksi berbasis sensor dan Internet of Things (IoT) \cite{bahga2014,Jardine2006Machinery}. Hasil penelitian diharapkan dapat menjadi referensi bagi penelitian selanjutnya mengenai diagnosis kerusakan motor induksi, pengembangan algoritma analisis kondisi motor, maupun implementasi sistem monitoring pada peralatan industri lainnya.
